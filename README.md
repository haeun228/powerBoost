...
#### 💡3주차
- refresh 토큰도 구현하고 싶었으나 어려워서 하지 못했다. 대신 로그인이 필요한 페이지를 접속할 때마다 토큰을 재발급 해주는 방식으로 구현했다.
- Express.js는 라우트 정의 순서가 중요하다
    - 검색 기능을 구현해서 API를 보내서 확인할 때 \`search?query=~~\` 부분이 특정 게시물을 가져오는 API인 \`GET /posts/:id\` 의 id 부분으로 넘어가서 처리되는 문제 발생
